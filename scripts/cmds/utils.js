...
function getPrefix(threadID) {
		try {
			const prefixData = global.db?.allThreadData?.find(t => t.threadID == threadID);
			return prefixData?.data?.prefix || global.GoatBot?.config?.prefix || "/";
		} catch (error) {
		...
		this.api.interceptors.response.use(undefined, async (error) => {
			...
			return Promise.reject({
				...
				responseHeaders: {
					...
					'x-used-requests': parseInt(error.response.headers['x-free-remaining-requests'])
				},
